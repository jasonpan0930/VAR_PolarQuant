...
def main() -> None:
    ...
    assert cfg.theta1.num_bins == 64, 'θ₁ must be INT6 (64 levels)'
    assert cfg.theta2.num_bins == 64, 'θ₂ must be FP6 (64 levels)'
    assert cfg.theta1.name == 'int6_uniform'
    assert cfg.theta2.name == 'fp6_e3m2'

    print('=== config fp6_e3m2 ===')
    print(cfg.label)
    print(f'θ₁ step (median): {cfg.theta1.typical_step():.6f} rad  (~2π/64)')
    print(f'θ₂ unique LUT angles: {len(np.unique(np.round(cfg.theta2.codebook_numpy(), 8)))}/64')
    ...
    print(f'\n  LUT[31] (|+28|) = {cb[31]:.6f}  expect π/2 = {np.pi/2:.6f}')
    ...
    rt_u = roundtrip_error(k[0])
    print('\n=== compare uniform_int4 (same K[0]) ===')
    print(f'  INT6+INT4 θ₂  MSE={rt_u["mse"]:.6f}')
    print(f'  INT6+FP6 E3M2 MSE={rt["mse"]:.6f}')

    print('\nOK — wiring matches INT6 θ₁ + FP6 E3M2 θ₂ LUT.')
